...
public class UserBanCreateUseCase {
    ...
    private UserBanCreateResponse executeInternal(UserBanCreateRequest request) {
        validate(request);
        LocalDateTime now = LocalDateTime.now(clock);
        User admin = requireAdmin(request.adminId());
        User target = requireActiveUser(request.userId(), "차단 대상 유저를 찾을 수 없습니다.");
        ...
        LocalDateTime bannedUntil = bannedUntil(request.banDurationDays(), now);
        UserBan saved = userBanRepository.save(
                UserBan.create(target, admin, request.banReason(), bannedUntil)
        );
    ...
    private void validate(UserBanCreateRequest request) {
        if (request == null || request.userId() == null || request.adminId() == null) {
        ...
        if (request.banReason() == null || request.banReason().isBlank()) {
            throw validation("차단 사유를 입력해 주세요.");
        }
    }

    private User requireAdmin(Long adminId) {
        User admin = requireActiveUser(adminId, "존재하지 않는 사용자입니다.");
    ...
    private User requireActiveUser(Long userId, String message) {
        return userRepository.findById(userId)
                ...
                .orElseThrow(() -> new UserApplicationException(UserErrorCode.USER_NOT_FOUND, message));
    }

    private LocalDateTime bannedUntil(int banDurationDays, LocalDateTime now) {
