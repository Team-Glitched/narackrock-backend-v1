...
class ShortsControllerTest {
    ...
    void GET_shorts_미인증_요청시_200을_반환한다() throws Exception {
        when(getShortsUseCase.get(isNull(), eq(ShortStatus.COMPLETED), eq(20), isNull()))
    ...
    void cursor_파라미터를_전달하면_유스케이스에_전달된다() throws Exception {
        when(getShortsUseCase.get(eq(50L), any(), anyInt(), isNull()))
                .thenReturn(emptyShortsPage(20));

        mockMvc.perform(get("/api/v1/shorts").param("cursor", "50"))
                .andExpect(status().isOk());

        verify(getShortsUseCase).get(eq(50L), any(), anyInt(), isNull());
    }

    @Test
    void size_파라미터를_전달하면_유스케이스에_전달된다() throws Exception {
        when(getShortsUseCase.get(isNull(), any(), eq(5), isNull()))
                .thenReturn(emptyShortsPage(5));

        mockMvc.perform(get("/api/v1/shorts").param("size", "5"))
                .andExpect(status().isOk());

        verify(getShortsUseCase).get(isNull(), any(), eq(5), isNull());
    }

    @Test
    void status_파라미터를_전달하면_유스케이스에_전달된다() throws Exception {
        when(getShortsUseCase.get(isNull(), eq(ShortStatus.IN_PROGRESS), anyInt(), isNull()))
                .thenReturn(emptyShortsPage(20));

        mockMvc.perform(get("/api/v1/shorts").param("status", "IN_PROGRESS"))
                .andExpect(status().isOk());

        verify(getShortsUseCase).get(isNull(), eq(ShortStatus.IN_PROGRESS), anyInt(), isNull());
    }
